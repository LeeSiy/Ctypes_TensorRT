...
int TensorRT::run() {
    ...
    while(i<1)
    {
            ...
            std::cerr << "TRT Engine Start..." << std::endl;
            cv::Mat img = cv::imread("/home/user/sylee/crnn/build/result.png");
            if (img.empty()) {
    ...
    std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start2).count() << "ms (10000 cycle)" << std::endl;
            /*
...
int run() {
    ...
    std::ifstream file("/home/user/SVN/AIEye_Airport_Prevention/src/AIsrc/AnalyzeServer/smarteye2/ocr_trt/crnn.engine", std::ios::binary);
    if (file.good()) {
        file.seekg(0, file.end);
        size = file.tellg();
        file.seekg(0, file.beg);
    ...
    cv::Mat img = cv::imread("/home/user/delete/tensorrtx/demo.png");
    if (img.empty()) {
        std::cerr << "demo.png not found !!!" << std::endl;
        return -1;
    }
...
py::array_t<unsigned char> test_rgb_to_gray(py::array_t<unsigned char>& input) {
    ...
    cv::Mat dst;
    return cv_mat_uint8_3c_to_numpy(img_rgb);

}



PYBIND11_MODULE(crnn, m) {
    // optional module docstring
    m.doc() = "pybind11 example crnn";

    // define add function
    m.def("run", &run, "Run crnn...");


    m.def("test_rgb_to_gray", &test_rgb_to_gray);

    py::class_<TensorRT>(m, "TensorRT")
        .def(py::init<>())
        .def("run", &TensorRT::run);
